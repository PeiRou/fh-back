...
$(function () {
    ...
    $('#menu-agentManage-review').addClass('active');
    ...
    dataTable = $('#capitalDetailsTable').DataTable({
        ...
        aLengthMenu: [[50]],
        ajax: {
            url:'/back/datatables/agentSettle/review',
            data:function (d) {
                d.account = $('#account').val();
                d.status = $('#status').val();
                d.startTime = $('#timeStart').val();
                d.endTime = $('#timeEnd').val();
                d.chkTest = $('#chkTest').is(':checked');
        ...
        columns: [
            ...
            {data:'base_fee'},
            {data:'fee_bunko'},
            {data:'month3_fee_bunko'},
            {data:'fenhong_prop'},
            {data:'commission'},
            {data:'month3_commission'},
            {data:'status'},
            {data:'control'},
        ],
        columnDefs: [ {
...
function settlement() {
    $.ajax({
        url:'/action/admin/agentSettle/settlement',
        type:'post',
        dataType:'json',
...
function edit(id) {
    jc = $.confirm({
        ...
        closeIcon:true,
        boxWidth:'46%',
        content: 'url:/back/modal/editAgentSettleReview/'+id,
        buttons: {
            formSubmit: {
                text:'确定修改',
                btnClass: 'btn-blue',
...
function editSettle(id) {
    $.ajax({
        url:'/action/admin/agentSettle/submitSettle',
        type:'post',
        dataType:'json',
...
function editTurnDown(id) {
    $.ajax({
        url:'/action/admin/agentSettle/submitTurnDown',
        type:'post',
        dataType:'json',
        data:{agent_report_idx:id},
        success:function (data) {
