$(function () {
    ...
    $('#menu-payNewManage-alipaySm').addClass('active');

    $('#payAlipayTable').DataTable({
        searching: false,
        bLengthChange: false,
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: '/back/datatables/payAlipaySmNew',
        columns: [
            {data:'payeeName'},
            {data:'payee'},
            {data:'qrCode'},
...
function add() {
    jc = $.confirm({
        ...
        closeIcon:true,
        boxWidth:'26%',
        content: 'url:/back/modal/addPayAlipaySmNew',
        buttons: {
            formSubmit: {
                text:'确定提交',
                btnClass: 'btn-blue',
                action: function () {
                    var form = this.$content.find('#addPayAlipayForm').data('formValidation').validate().isValid();
                    if(!form){
                        return false;
                    }
                    return false;
...
function edit(id) {
    jc = $.confirm({
        ...
        closeIcon:true,
        boxWidth:'26%',
        content: 'url:/back/modal/editPayAlipaySmNew/'+id,
        buttons: {
            formSubmit: {
                text:'确定提交',
                btnClass: 'btn-blue',
                action: function () {
                    var form = this.$content.find('#editPayAlipayForm').data('formValidation').validate().isValid();
                    if(!form){
                        return false;
                    }
                    return false;
