angular.module('ionicz.lottery')
    ...
    .filter('lotteryResult', function($interpolate) {
        return function(nums, group) {
            ...
            else if (group == 'group9') {
                results.push((nums[0] >=5 ) ? '大' : '小'); // 1
                results.push((nums[1] >=5 ) ? '大' : '小'); // 1
                results.push((nums[2] >=5 ) ? '大' : '小'); // 1
            }
    ...
    // 聊天室背景渐变过滤
    .filter('chatBgColor', [function() {
        return function(color) {
            var cutArr = color.split(';');
            if (cutArr.length == 2) {
                return 'linear-gradient(to bottom,' + cutArr[0] + ',' + cutArr[1] + ')';
            } else {
                return color;
            }
    ...
    // 聊天室背景尖角色
    .filter('chatBorderColor', [function() {
        return function(color) {
            var cutArr = color.split(';');
            if (cutArr.length == 2) {
                return cutArr[0];
