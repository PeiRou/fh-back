angular.module('ionicz.lottery')
    ...
    .controller('ListCtrl', function($scope, $window, $log, Lottery, $ionicPopover, $timeout) {
        $log.debug("ListCtrl...");

        $ionicPopover.fromTemplateUrl('/mobile/views/lottery/templates/list-poper.html', {
            scope: $scope
        ...
        $scope.$on('$ionicView.afterEnter', function() {
            $scope.$broadcast('lotteryListEnter');
        });

        $scope.$on('$destroy', function() {
            $log.debug('游戏大厅Ctrl销毁，发送广播消息，用于触发销毁定时器');
            $scope.$broadcast('lotteryListLeave');
        });

        /*$scope.$on('$ionicView.beforeLeave', function() {
        	$log.debug('游戏大厅处于非激活状态，发送广播消息，用于触发销毁定时器');
            $scope.$broadcast('lotteryListLeave');
        });	*/
    })

    .controller('LotteryCtrl', function($scope, $timeout, $log, $filter, $stateParams, $ionicPopover, Lottery, Tools, My, $ionicScrollDelegate,Chat) {
        $log.debug("LotteryCtrl...");

        $scope.noRoom = function () {
            Tools.tip('聊天室暂未启用');
        };

        $scope.showChouMa = false;
        ...
        $scope.show_chouma = function() {
            $scope.showChouMa = $scope.showChouMa === false ? true: false;
        };

        Lottery.initPlayData(function() {
            $scope.$broadcast('lotteryPlayInited');
        });

        $scope.$on('$ionicView.afterEnter', function() {
            $scope.$broadcast('lotteryEnter');
            Lottery.resetGame($scope.gameId);
            $scope.reset();
        });
        ...
        $scope.$on('$destroy', function() {
            ...
            $scope.$broadcast('lotteryLeave');
        });

        // 缓存模式触发非激活事件
        $scope.$on('$ionicView.beforeLeave', function() {
            $log.debug('游戏处于非激活状态，发送广播消息，用于触发销毁定时器');
            $scope.$broadcast('lotteryLeave');
        });
        ...
        $ionicPopover.fromTemplateUrl('/mobile/views/lottery/templates/lottery-poper.html', {
            scope: $scope
        }).then(function(popover) {
            $scope.popover = popover;
        });
        ...
        $scope.showRole = function() {
            Tools.modal({
                ...
                templateUrl: "/mobile/views/lottery/templates/role/role_" + $scope.gameId + ".html",
                css: 'role-poput',
        ...
        $scope.bet = function() {
            ...
            if ($scope.selectSub) {
            	// 如果是连肖或者连尾
                if ($scope.selectSub.type == 'LX' || $scope.selectSub.type == 'LW') {
                    ...
                    for (var i in betSubArray) {
                        ...
                        for (var j in playIdArray) {
                            ...
                            else if ($scope.selectSub.type == 'LX' && play.odds < odds) {
                                odds = play.odds;
                                playId = play.id;
                            }
                            else if ($scope.selectSub.type == 'LW' && play.odds > odds) {
                                odds = play.odds;
                                playId = play.id;
                            }
                ...
                for (var i = 0; i < playIdArray.length; i++) {
                    ...
                    betParams["betBean_playId[" + index + "]"] = playId;
                    betParams["betBean_odds[" + index + "]"] = play.odds;
                    betParams["betBean_rebate[" + index + "]"] = play.rebate;
                    betParams["betBean_money[" + index + "]"] = betMoney;
                    confirmHtml += '<div>【' + Lottery.getPlayName(play) + '】 @' + play.odds + ' x ' + betMoney + '</div>';
                    index++;
                }
            ...
            Tools.modal({
                ...
                callback: function(scope, popup) {
                    ...
                    Tools.ajax({
                        url: '/api/lottery/bet.do',
                        // params: betParams,
                        params: {data:JSON.stringify(betParams)},
                        backdrop: true,
                        success: function() {
                            Tools.tip('下注成功');
                            $scope.reset();
        ...
        $scope.chat = Chat;
        //滚动绑定
        $scope.$on('repeat-finish', function() {
            $scope.chat.toBottom();
            // $scope.chat.faceShow();
        })
        ...
        $scope.sendImage = function(e) {
            ...
            if ( /\.(jpe?g|png|gif)$/i.test(file.name) ) {
                $scope.chat.sendImage(file);
            }
        }

        $scope.sendAvatar = function(e) {
            var file = file = e.target.files[0];
            if ( /\.(jpe?g|png|gif)$/i.test(file.name) ) {
                $scope.chat.sendAvatar(file);
            }
        }
    })
    ...
    .controller('NotcountCtrl', function($scope, $log, Tools, Lottery, $state) {
        ...
        $scope.onQuery = function() {
            Tools.ajax({
                url: '/api/mobile/lottery/getNotcount',
                method: 'GET',
    ...
    .controller('NotcountDetailCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state) {
        ...
        $scope.getPlayName=function(play){
        	return Lottery.getPlayName(play);
        };

        $scope.onQuery = function() {
            Tools.ajax({
                url: '/api/mobile/lottery/getNotcountDetail',
                method: 'GET',
                backdorp: true,
    ...
    .controller('SettledCtrl', function($scope, $log, Tools, Lottery, $state, $filter, $timeout) {
        ...
        $scope.onQuery = function() {
            ...
            $scope.isMore = false;
            Tools.ajax({
                url: '/api/mobile/lottery/getSettled',
                method: 'GET',
                backdorp: true,
                ...
                success: function(result) {
                    ...
                    if (result && result.totalCount > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            dataList.push({ turnNum: game.name + '</br>' + data[i].turnNum, money: data[i].money, resultMoney: data[i].resultMoney, detail: Lottery.getPlayName(play) + ' ' + data[i].betInfo + '</br>@' + play.odds + '</br>#' + data[i].rebate, rebate: data[i].rebate });
                        }
    ...
    .controller('HistoryCtrl', function($scope, $log, $stateParams, $state, Tools, Lottery) {
        ...
        var getHistoryUrl = function() {
            var date = $scope.queryData.date;
            if (!$scope.showDate || $scope.queryData.gameId == 70 || $scope.queryData.gameId == 30) {
                date = '';
                if ($scope.queryData.gameId != 70 && $scope.queryData.gameId != 30) {
                    date = moment().format('YYYYMMDD');
                }
            }

            return '/api/mobile/lotteryHistory/data?date=' + date + '&gameId=' + $scope.queryData.gameId +'&_' + Math.random();
        };
        ...
        // 单独处理，如果是凌晨5点前，默认查询前一天的日期
        if (($scope.queryData.gameId == 55 || $scope.queryData.gameId == 80 || $scope.queryData.gameId == 81 || $scope.queryData.gameId == 82|| $scope.queryData.gameId == 83|| $scope.queryData.gameId == 84|| $scope.queryData.gameId == 85) && moment().hour() < 5) {
    ...
    .controller('ChangLongCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state) {
        ...
        $scope.onQuery = function() {
            Tools.ajax({
                url: Tools.staticPath() + 'data/' + $scope.queryData.gameId + 'stat.json?_' + Math.random(),
                method: 'GET',
                success: function(curStatList) {
    ...
    .controller('LuZhuCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state) {
        ...
        $scope.queryData = {};
        $scope.queryData.gameId = parseInt($stateParams.gameId || Lottery.getFirstGameId());
        if ($scope.queryData.gameId == 10 || $scope.queryData.gameId == 70 || $scope.queryData.gameId == 30 || $scope.queryData.gameId == 66 || $scope.queryData.gameId == 84 || $scope.queryData.gameId == 85) {
            Tools.tip('无此类游戏路珠');
        ...
        loadData = function() {
            Tools.ajax({
                url: Tools.staticPath() + 'data/' + $scope.queryData.gameId + 'stat_game_mobile.json?_' + Math.random(),
                method: 'GET',
                success: function(luZhuData) {
    ...
    .controller('WeekRecordCtrl', function($rootScope, $scope, $state, Tools, My) {
        ...
        var onQuery = function() {
            Tools.ajax({
                ...
                url: '/api/mobile/user/getStatBets',
                success: function(result) {
                    if (!result) {
                        return;
                    }
    ...
    .controller('DayRecordCtrl', function($scope, $log, Tools, Lottery, $state, $stateParams) {
        ...
        $scope.onQuery = function() {
            Tools.ajax({
                url: '/lottery/getTotalStatBets.do',
                method: 'GET',
