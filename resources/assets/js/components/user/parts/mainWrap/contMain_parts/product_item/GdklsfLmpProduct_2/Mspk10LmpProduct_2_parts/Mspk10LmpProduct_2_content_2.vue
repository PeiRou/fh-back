<template>
    ...
    <!--需要先放前面4个-->
    <Mspk10LmpProduct_2_content_item v-for="(item,index) in info.productInfo.productContent" v-if="item.playCateId === info.productId && isNaN(parseInt(item.code)) && item.id > 607920 && item.code != 'DONG' && item.code != 'NAN' && item.code != 'XI' && item.code != 'BEI' && item.code != 'ZHONG' && item.code != 'FA' && item.code != 'BAI'" :info="item" :key="index"></Mspk10LmpProduct_2_content_item>

    <!--<Mspk10LmpProduct_2_content_item v-for="(item,index) in info.productInfo.productContent" v-if="item.playCateId === info.productId && isNaN(parseInt(item.code)) && item.id > 607921 && item.code != 'DONG' && item.code != 'NAN' && item.code != 'XI' && item.code != 'BEI' && item.code != 'ZHONG' && item.code != 'FA' && item.code != 'BAI'" :info="item" :key="index"></Mspk10LmpProduct_2_content_item>-->
