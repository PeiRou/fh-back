<template>
    <tbody>
    <!--<Mspk10LmpProduct_2_content_item v-for="item in Object.values(info)" :info="info"></Mspk10LmpProduct_2_content_item>-->
    <!--{{info}}-->
    <Mspk10LmpProduct_2_content_item v-for="(item,index) in Object.values(info)[0]" :info="item" :key="index"></Mspk10LmpProduct_2_content_item>
    </tbody>

    <!--<tr v-for='(item,index) in info.productInfo.productContent' v-if="item.id === info.productId && isNaN(parseInt(item.code))">-->
    <!--<Mspk10LmpProduct_2_content_item v-for="(item,index) in info_productInfo_productContent_limitBy4" v-if="item.playCateId === info.productId && isNaN(parseInt(item.code))" :info="item" :key="index"></Mspk10LmpProduct_2_content_item>-->
    <!--{{Xykl8LmpInput}}-->
    <!--<Mspk10LmpProduct_2_content_item v-for="(item,index) in info_productInfo_productContent_limitBy4" v-if="item.playCateId === info.productId && isNaN(parseInt(item.code))" :info="item" :key="index"></Mspk10LmpProduct_2_content_item>-->
...
<script>
  import { mapGetters } from 'vuex'
    ...
    export default {
        ...
        computed: {
          ...mapGetters({
            Xykl8LmpInput: 'getXykl8LmpPlays_196_title'
          })
        }
        // computed: {
        //   info_productInfo_productContent_limitBy4: function () {
        //     return this.info.productInfo.productContent.slice(0,4)
        //   }
        // }
    }
</script>
