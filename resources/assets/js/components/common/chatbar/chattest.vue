                            ...
                            <div class="control-bar">
                                ...
                                <label for="imgUploadInput" >
                                    <span title="上传图片" class="btn-control" >
                                        ...
                                        <input id="imgUploadInput" :disabled="!platcfg.is_open"  @change="upload" type="file" accept=".jpg, .png, .gif, .jpeg, image/jpeg, image/png, image/gif" style="width: 0.1px; height: 0.1px; opacity: 0; position: absolute;">
                                    </span>
                                </label>
...
<script>
    ...
    export default  {
            ...
            login:function(){
                ...
                window.axios.post('chat/login').then(function (response) {
                    ...
                    for(var i=0 ; i<res.messages.length; i++) {
                        messageList.push(JSON.parse(res.messages[i]));
                    }
                    if(messageList.length>0){
                        messageList.push({type:'SysMsg'});
                    }
                    messageList.push({type:'UserNickSysMsg'});
                    messageList.push({type:'UserAvatarSysMsg'});
        ...
        mounted () {
            ...
            // /**接收开奖信息**/
            // this.socket.on('open-channel',function (data) {
            //     console.log(data);
            // });
    ...
    //       chatIsTrue : true,
    //         // 聊天室默认的页面
    //         // chatBarUrl: 'http://112.213.105.60:8002/'
    //         chatBarUrl: 'http://112.213.105.60:8002/'
    //
    //     }
    //   },
