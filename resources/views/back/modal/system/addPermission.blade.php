...
<script>
    $(function () {
        ...
        //     { id:904, pId:9, name:"文章管理",value:'system.articleManage'},
        //
        ...
        var zNodes = [
            @foreach($aPermissionsAuths as $aPermissionsAuth)
                { id : '{{ $aPermissionsAuth->id }}',pId : '{{ $aPermissionsAuth->pid }}',name : '{{ $aPermissionsAuth->auth_name }}',value : '{{ $aPermissionsAuth->route_name }}'},
                @if(!empty($aPermissionsAuth->child))
                    @foreach($aPermissionsAuth->child as $child)
                        { id : '{{ $child->id }}',pId : '{{ $child->pid }}',name : '{{ $child->auth_name }}',value : '{{ $child->route_name }}'},
                        @if(!empty($child->child))
                            @foreach($child->child as $child2)
                                { id : '{{ $child2->id }}',pId : '{{ $child2->pid }}',name : '{{ $child2->auth_name }}',value : '{{ $child2->route_name }}'},
                            @endforeach
                        @endif
                    @endforeach
                @endif
