...
<form id="gameSettingForm" class="ui form" action="{{ url('/action/admin/editAgentOdds') }}">
    <div class="field">
        <label>赔率(不得高于{{ $iOdds }})</label>
        <div class="ui input icon">
            <input type="text" name="odds" value="{{ $iData->odds }}" placeholder="请填写赔率,不得高于{{ $iOdds }}"/>
        </div>
    </div>
    <input type="hidden" value="{{ $iData->id }}" name="id">
</form>
<script>
$(function () {
