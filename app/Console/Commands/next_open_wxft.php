...
class next_open_wxft extends Command
{
    protected  $code = 'wxft';
     ...
     * @var string
     */
    protected $signature = 'next_open_wxft';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('wxft:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('wxft:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('wxft:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('wxft:gapnum',$gapnum);
                        $this->clong->setKaijian('wxft',1,$opencode);
                        $this->clong->setKaijian('wxft',2,$opencode);
                    }
