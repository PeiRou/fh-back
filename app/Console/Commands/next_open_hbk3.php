...
class next_open_hbk3 extends Command
{
    protected  $code = 'hbk3';
    protected  $gameId = 13;
    /**
     ...
     * @var string
     */
    protected $signature = 'next_open_hbk3';
     ...
     * @var string
     */
    protected $description = '湖北快3-定時開號';
    ...
    public function handle()
    {
        ...
        $redis_needopen = $redis->exists('hbk3:needopen')?$redis->get('hbk3:needopen'):'';
        $redis_next_issue = $redis->get('hbk3:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('hbk3:gapnum');
        //在現在實際的差距
        ...
        if($needOpenIssue == $redis_issue)
            $url = Config::get('website.guanIssueServerUrl').'hbk3';
        else
            $url = Config::get('website.guanIssueServerUrl').'hbk3?issue='.$needOpenIssue;
        try {
