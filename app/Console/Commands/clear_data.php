...
//        $redis->setex('clearing',300,'on');
        ...
        if(!is_dir($path))
            mkdir($path);
        if(is_file($path.'/clearing.txt')){
            echo "ing...";
            return "";
        }
        $fpClearing = fopen($path.'/clearing.txt','a+');
        if(flock($fpClearing,LOCK_EX)) {
...
//            if (!$redis->exists('clear-bet')) {
            if(!is_file($path.'/clearBet.txt')) {
                $fpClearBet = fopen($path . '/clearBet.txt', 'a+');
                if (flock($fpClearBet, LOCK_EX)) {
...
//                        $redis->setex('clear-bet', 1, 'on');
                            flock($fpClearing, LOCK_UN);
                            @unlink($path . '/clearBet.txt');
                            echo '2';
                        }
                    }
                }
...
//            if (!$redis->exists('clear-bet-his')) {
            if(!is_file($path . '/clearBetHis.txt')) {
                $fpClearBetHis = fopen($path . '/clearBetHis.txt', 'a+');
                if (flock($fpClearBetHis, LOCK_EX)) {
...
//                        $redis->setex('clear-bet-his', 1, 'on');
                        flock($fpClearBetHis, LOCK_UN);
                        @unlink($path . '/clearBetHis.txt');
                    }
                }
                fclose($fpClearBetHis);
            }

            //清-资金明细
//            if (!$redis->exists('clear-else')) {
            if(!is_file($path . '/clearElse.txt')) {
                $fpClearElse = fopen($path . '/clearElse.txt', 'a+');
                if (flock($fpClearElse, LOCK_EX)) {
                    $num_else = 0;
...
//                    $redis->setex('clear-else', 1, 'on');
                        flock($fpClearElse, LOCK_UN);
                        @unlink($path . '/clearElse.txt');
                        $num++;
                    }
                }
                fclose($fpClearElse);
            ...
            //清-棋牌昨日数据
            if(!is_file($path . '/clearJqBet.txt')) {
                $fpClearJqBet = fopen($path . '/clearJqBet.txt', 'a+');
//            if (!$redis->exists('clear-jq-bet')) {
                if (flock($fpClearJqBet, LOCK_EX)) {
...
//                    $redis->setex('clear-jq-bet', 1, 'on');
                        flock($fpClearJqBet, LOCK_UN);
                        @unlink($path . '/clearJqBet.txt');
                    }
                }
                fclose($fpClearJqBet);
            }
            //清-棋牌历史数据
            if(!is_file($path.'/clearJqBetHis.txt')) {
                $fpClearJqBetHis = fopen($path . '/clearJqBetHis.txt', 'a+');
//            if (!$redis->exists('clear-jq-bet-his')) {
                if (flock($fpClearJqBetHis, LOCK_EX)) {
...
//                    $redis->setex('clear-jq-bet-his', 1, 'on');
                        flock($fpClearJqBetHis, LOCK_UN);
                        @unlink($path.'/clearJqBetHis.txt');
                    }
                }
                fclose($fpClearJqBetHis);
            }
            if ($num == 0) {
