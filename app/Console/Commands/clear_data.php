...
//        $this->stoptime = date('Y-m-d H:i:s',strtotime(date('Y-m-d 23:59:59'))+3600);                                 //卡redis时间，改成一点之后才开始移数据
        ...
        if(!$redis->exists('clear-else')) {
            ...
            //特定休市彩种删奖期
            $res = ['game_bjpk10','game_pcdd','game_gd11x5','game_bjkl8','game_gdklsf','game_xjssc','game_pknn','game_ahk3','game_gxk3','game_hbk3','game_hebeik3','game_gsk3','game_gzk3','game_twbg28','game_twbgc'];
            foreach ($res as $key => $table){
                $num_else = $this->delGameTables($table, '2020-01-22 00:00:00', '2020-03-11 23:59:59', $num_else);
            }
            //清-计画试算
