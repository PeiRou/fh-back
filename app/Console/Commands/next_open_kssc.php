...
class next_open_kssc extends Command
{
    protected  $code = 'kssc';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('kssc:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('kssc:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('kssc:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('kssc:gapnum',$gapnum);
                        $this->clong->setKaijian('kssc',1,$opencode);
                        $this->clong->setKaijian('kssc',2,$opencode);
                    }
