...
class ISSUE_SEED_MSSSC extends Command
{
    ...
    protected $description = '秒速时时彩期数生成-985';
    ...
    public function handle()
    {
        ...
        if($checkUpdate->msssc == $curDate){
            ...
            if($run == 1){
                ...
                if($update == 1){
                    $delClong1 = DB::table('clong_kaijian1')->where('lotteryid',81)->delete();
                    if($delClong1 == 1){
                        $delClong2 = DB::table('clong_kaijian2')->where('lotteryid',81)->delete();
                        if($delClong2 == 1){
