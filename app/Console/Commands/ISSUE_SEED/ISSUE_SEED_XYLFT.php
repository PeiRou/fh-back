...
class ISSUE_SEED_XYLFT extends Command
{
    protected $signature = 'ISSUE_SEED_XYLFT';
    ...
    public function handle()
    {
        ...
        $issueDate = '';
        if(isset($checkUpdate->xylft)) {
            if($curDate == $checkUpdate->xylft) {
                $issueDate = date('Y-m-d', strtotime('+ 1 day', time()));
                $curDate = date('ymd', strtotime('+ 1 day', time()));
            }else if($curDate < $checkUpdate->xylft)
        ...
        $sql = "INSERT INTO game_xylft (issue,opentime) VALUES ";
        for($i=1;$i<=276;$i++){
