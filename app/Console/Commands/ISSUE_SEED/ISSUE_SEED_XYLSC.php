...
class ISSUE_SEED_XYLSC extends Command
{
    protected $signature = 'ISSUE_SEED_XYLSC';
    protected $description = '匈牙利赛车期数生成-276';
    ...
    public function handle()
    {
        ...
        $issueDate = '';
        if(isset($checkUpdate->xylsc)) {
            if($curDate == $checkUpdate->xylsc) {
                $issueDate = date('Y-m-d', strtotime('+ 1 day', time()));
                $curDate = date('ymd', strtotime('+ 1 day', time()));
            }else if($curDate < $checkUpdate->xylsc)
        ...
        $sql = "INSERT INTO game_xylsc (issue,opentime) VALUES ";
        for($i=1;$i<=276;$i++){
