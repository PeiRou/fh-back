...
class ISSUE_SEED_XY28 extends Command
{
    protected $signature = 'ISSUE_SEED_XY28';
    protected $description = '幸运28期数生成-270';
    ...
    public function handle()
    {
        ...
        $issueDate = '';
        if(isset($checkUpdate->xy28)){
            if($curDate == $checkUpdate->xy28) {
                $issueDate = date('Y-m-d', strtotime('+ 1 day', time()));
                $curDate = date('ymd', strtotime('+ 1 day', time()));
            }else if($curDate < $checkUpdate->xy28)
        ...
        $sql = "INSERT INTO game_xy28 (issue,opentime) VALUES ";
        for($i=1;$i<=270;$i++){
