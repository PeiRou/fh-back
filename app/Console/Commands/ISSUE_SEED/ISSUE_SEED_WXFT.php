...
class ISSUE_SEED_WXFT extends Command
{
    protected $signature = 'ISSUE_SEED_WXFT';
    protected $description = '无限飞艇期数生成-276';
    ...
    public function handle()
    {
        ...
        $issueDate = '';
        if(isset($checkUpdate->wxft)) {
            if($curDate == $checkUpdate->wxft) {
                $issueDate = date('Y-m-d', strtotime('+ 1 day', time()));
                $curDate = date('ymd', strtotime('+ 1 day', time()));
            }else if($curDate < $checkUpdate->wxft)
        ...
        $sql = "INSERT INTO game_wxft (issue,opentime) VALUES ";
        for($i=1;$i<=276;$i++){
