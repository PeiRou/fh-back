...
class ISSUE_SEED_SFSSC extends Command
{
    protected $signature = 'ISSUE_SEED_SFSSC';
    protected $description = '三分时时彩期数生成-480期-24小时';
    ...
    public function handle()
    {
        ...
        $issueDate = '';
        if(isset($checkUpdate->sfssc)) {
            if($curDate == $checkUpdate->sfssc) {
                $issueDate = date('Y-m-d', strtotime('+ 1 day', time()));
                $curDate = date('ymd', strtotime('+ 1 day', time()));
            }else if($curDate < $checkUpdate->sfssc)
        ...
        $sql = "INSERT INTO game_sfssc (issue,opentime) VALUES ";
        for($i=1;$i<=480;$i++){
