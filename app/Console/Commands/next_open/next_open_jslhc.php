...
class next_open_jslhc extends Command
{
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('jslhc:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('jslhc:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('jslhc:gapnum');
        //在現在實際的差距
        ...
        try {
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    $up = DB::table('game_jslhc')->where('issue',$needOpenIssue)->update([
                        'is_open' => 1,
