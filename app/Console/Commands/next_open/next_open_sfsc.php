...
class next_open_sfsc extends Command
{
    protected  $code = 'sfsc';
    protected  $gameId = 901;
    protected  $clong;
    protected $signature = 'next_open_sfsc';
    protected $description = '三分赛车-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('sfsc:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('sfsc:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('sfsc:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('sfsc:gapnum',$gapnum);
                        $this->clong->setKaijian('sfsc',1,$opencode);
                        $this->clong->setKaijian('sfsc',2,$opencode);
                    }
