...
class next_open_hlsx extends Command
{
    protected  $code = 'hlsx';
    protected  $gameId = 2;
    protected  $clong;
    protected $signature = 'next_open_hlsx';
    protected $description = '欢乐生肖-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('hlsx:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('hlsx:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('hlsx:gapnum');
        //在現在實際的差距
            ...
            if (isset($html['issue']) && $redis_issue !== $html['issue']) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('hlsx:gapnum',$gapnum);
                        $this->clong->setKaijian('hlsx',1,$html['nums']);
                        $this->clong->setKaijian('hlsx', 2, $html['nums']);
                    }
