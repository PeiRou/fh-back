...
class next_open_twbg28 extends Command
{
    protected  $code = 'twbg28';
    protected  $gameId = 43;
    protected  $clong;
    protected $signature = 'next_open_twbg28';
    protected $description = '台湾宾果28-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('twbg28:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('twbg28:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('twbg28:gapnum');
        //在現在實際的差距
        ...
        if(empty($res)){
            ...
            $redis->set('twbg28:gapnum',$gapnum);
            return 'Fail';
        }else{
            //阻止進行中
            ...
            if (isset($html['issue']) && $redis_issue !== $html['issue']) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('twbg28:gapnum',$gapnum);
                        $this->clong->setKaijian('twbg28', 2, $html['nums']);
                    }
