...
class next_open_xyft extends Command
{
    protected  $code = 'xyft';
    protected  $gameId = 55;
    protected  $clong;
    protected $signature = 'next_open_xyft';
    protected $description = '幸运飞艇-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('xyft:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('xyft:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('xyft:gapnum');
        //在現在實際的差距
            ...
            if (isset($html['issue']) && $redis_issue !== $html['issue']) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('xyft:gapnum',$gapnum);
                        $this->clong->setKaijian('xyft',1,$html['nums']);
                        $this->clong->setKaijian('xyft',2,$html['nums']);
                    }
