...
class next_open_xylsc extends Command
{
    protected  $code = 'xylsc';
    protected  $gameId = 905;
    protected  $clong;
    protected $signature = 'next_open_xylsc';
    protected $description = '匈牙利赛车-定时开号';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('xylsc:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('xylsc:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('xylsc:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('xylsc:gapnum',$gapnum);
                        $this->clong->setKaijian('xylsc',1,$opencode);
                        $this->clong->setKaijian('xylsc',2,$opencode);
                    }
