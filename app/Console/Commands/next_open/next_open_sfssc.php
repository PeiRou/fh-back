...
class next_open_sfssc extends Command
{
    protected  $code = 'sfssc';
    protected  $gameId = 902;
    protected  $clong;
    protected $signature = 'next_open_sfssc';
    protected $description = '三分时时彩-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('sfssc:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('sfssc:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('sfssc:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('sfssc:gapnum',$gapnum);
                        $this->clong->setKaijian('sfssc',1,$opencode);
                        $this->clong->setKaijian('sfssc',2,$opencode);
                    }
