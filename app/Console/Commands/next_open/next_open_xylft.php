...
class next_open_xylft extends Command
{
    protected  $code = 'xylft';
    protected  $gameId = 906;
    protected  $clong;
    protected $signature = 'next_open_xylft';
    protected $description = '匈牙利飞艇-定時開號';
    ...
    public function handle()
    {
        ...
        $redis->select(0);
        $redis_issue = $redis->get('xylft:issue');
        $redis_needopen = $redis->exists($this->code.':needopen')?$redis->get($this->code.':needopen'):'';
        $redis_next_issue = $redis->get('xylft:nextIssue');
        //在redis上的差距
        $redis_gapnum = $redis->get('xylft:gapnum');
        //在現在實際的差距
            ...
            if ($redis_issue !== $needOpenIssue) {
                try {
                    ...
                    if ($up == 1 && $needOpenIssue == ($redis_next_issue-1)) {
                        ...
                        $redis->set('xylft:gapnum',$gapnum);
                        $this->clong->setKaijian('xylft',1,$opencode);
                        $this->clong->setKaijian('xylft',2,$opencode);
                    }
