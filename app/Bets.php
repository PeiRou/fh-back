...
class Bets extends Model
{
    ...
    public static function getBetAndUserByIssueAll($issue,$gameId){
        $aSql = "SELECT `users`.`id`,SUM(`bet`.`bet_money`) AS `bet_money`,SUM(CASE WHEN `bet`.`game_id` IN(90,91) THEN `bet`.`nn_view_money` ELSE (CASE WHEN `bet`.`bunko` > 0 THEN `bet`.`bunko` - `bet`.`bet_money` ELSE `bet`.`bunko` END)END) AS `bet_bunko`,`bet`.`game_id`,`bet`.`issue`,`users`.`money`
                    ...
                    WHERE `bet`.`issue` = :issue AND `bet`.`game_id` = :game_id AND `bet`.`bunko` != 0
    ...
    public static function getBetUserDrawingByIssue($issue,$gameId){
        $aSql = "SELECT `users`.`id`,SUM(`bet`.`bet_money`) AS `bet_money`,SUM(CASE WHEN `bet`.`game_id` IN(90,91) THEN `bet`.`nn_view_money` ELSE (CASE WHEN `bet`.`bunko` > 0 THEN `bet`.`bunko` - `bet`.`bet_money` ELSE `bet`.`bunko` END)END) AS `bet_bunko`,`bet`.`game_id`,`bet`.`issue`,`users`.`money`,`dr`.`amount`
                    FROM `bet` 
                    JOIN `users` ON `users`.`id` = `bet`.`user_id`
...
//        $aSql1 = "SELECT count(b.bet_id) as bet_count,sum(b.bet_money) as bet_money,
//sum(case WHEN b.game_id in (90,91) then (case WHEN nn_view_money > 0 then bet_money else 0 end) else(case WHEN bunko >0 then bet_money else 0 end) end) as bet_amount,
//sum(case WHEN b.game_id in (90,91) then nn_view_money else(case when bunko >0 then bunko-bet_money else bunko end)end) as fact_bet_bunko,
//sum(case WHEN b.game_id in (90,91) then nn_view_money else(case when bunko >0 then bunko-bet_money else bunko end)end) as bet_bunko,
//'0.00' AS odds_amount,'0.00' AS return_amount,'0.00' AS fact_return_amount";
