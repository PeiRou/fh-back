...
class Agent extends Model
{
    ...
    public static function getAgentAllBunko($agentId){
        $aSql = 'SELECT `agent`.a_id,`agent`.account,`agent`.name,`agent`.created_at,0 AS `bunko`,`general_agent`.fenhong_rate
                FROM `agent` 
                INNER JOIN `agent` ON `agent`.gagent_id = `general_agent`.ga_id
