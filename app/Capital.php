...
//        't22' => '游戏撤销',
    ...
    public static function betMemberReportOtherData($startTime = '',$endTime = '', $types = []){
        ...
        $aSql = "SELECT
                    ...
                    SUM( CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `capital`.`money` ELSE 0 END ) AS cai_money,
    ...
    public static function betAgentReportOtherData($startTime = '',$endTime = '', $types = []){
        $aSql = "SELECT 
                    ...
                    SUM( CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `capital`.`money` ELSE 0 END ) AS cai_money,
                    COUNT(DISTINCT CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `capital`.`to_user` ELSE NULL END ) AS cai_count
                  FROM `capital`
                  JOIN `users` ON `users`.`id` = `capital`.`to_user`
                  WHERE 1
    ...
    public static function betGeneralReportOtherData($startTime = '',$endTime = '', $types = [])
    {
        $aSql = "SELECT 
                  ...
                  SUM( CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `capital`.`money` ELSE 0 END ) AS cai_money,
                  COUNT(DISTINCT CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `capital`.`to_user` ELSE NULL END ) AS cai_count,
                  COUNT(DISTINCT CASE WHEN `capital`.`type` = 't18' AND `capital`.`rechargesType` = 2 THEN `agent`.`a_id` ELSE NULL END ) AS cai_agent_count
                  FROM `capital`
                  JOIN `users` ON `users`.`id` = `capital`.`to_user`
                  JOIN `agent` ON `agent`.`a_id` = `users`.`agent`
