        ...
        $bet = Bets::select()
            ->where(function ($query) use ($searchType){
                if(isset($searchType) && $searchType){
                    if($searchType == 'yestoday'){
                        $query->whereRaw("to_days(now())-to_days(created_at)=1");
                    }
                }
            })
            ...
            ->where(function ($query) use ($maxMoney){
                if(isset($maxMoney) && $maxMoney){
                    $query->where("bet_money",'<=',$maxMoney);
                }
            })
...
//        return count($games);
        ...
        return DataTables::of($bet)
            ->editColumn('order_id',function ($bet){
                return '<span>'.$bet->order_id.'</span>';
            })
            ->editColumn('user',function ($bet) use ($user){
                return '<span>'.$user->username.'</span>';
            })
            ->editColumn('game',function ($bet){
                $game = DB::table('game')->where('game_id',$bet->game_id)->first();
                return '<span>'.$game->game_name.'</span>';
            })
            ->editColumn('issue',function ($bet){
                return '<span style="color: #'.$bet->color.'">'.$bet->issue.'</span>';
            })
            ->editColumn('play',function ($bet){
                if($bet->game_id == 1){
                    $weishu = strlen((int)$bet->game_id.$bet->playcate_id);
            ...
            ->editColumn('bet_money',function ($bet){
                return '<span class="green-text">'.$bet->bet_money.'</span>';
            })
            ...
            ->editColumn('bunko',function ($bet){
                ...
                if($bet->bunko < 0){
                    return '<span class="red-text">'.$bet->bunko.'</span>';
                } else {
                    return '<span class="blue-text">'.$bet->bunko.'</span>';
                }
            })
