...
//                $this->updateBatch1($id); //pc端生成table
    ...
    private function updateBatch1($id){
        ...
        foreach ($data as $item){
            ...
            foreach ($filter as $s){
                if($item->rebate_tag == $s){
                    $fromDBRebate->put($item->rebate_tag,[
                        'key' => $item->rebate,
                        'minMoney' => $item->minMoney,
                        'maxMoney' => $item->maxMoney
                    ]);
        ...
        $str = (view('table.'.$this->view_name)
            ->with('odds',$fromDBOdds->all())
            ->with('rebate',$fromDBRebate->all()));
        return Storage::disk('static')->put('tables/'.$id.'.json', $str);
    }
}
