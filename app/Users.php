...
class Users extends Model
{
    ...
    public static function exportUserData($aParam){
        $aSql = "SELECT `users`.`bank_name`,`users`.`content`,`users`.`bank_num`,`users`.`bank_addr`,`users`.`wechat`,`users`.`username`,`users`.`fullName`,`users`.`email`,`users`.`mobile`,`users`.`created_at`,`users`.`saveMoneyCount`,`users`.`lastLoginTime`,`re`.`sumReAmount`,`dr`.`sumDrAmount` FROM `users` 
LEFT JOIN (SELECT (CASE WHEN  `payType` != 'adminAddMoney' THEN (CASE WHEN SUM(amount) IS NULL THEN 0 ELSE SUM(amount) END) END) AS sumReAmount,(CASE WHEN  `payType` = 'adminAddMoney' THEN (CASE WHEN SUM(amount) IS NULL THEN 0 ELSE SUM(amount) END) END) AS sumReAmountAd,`userId` FROM `recharges` WHERE `status` = 2 GROUP BY `userId`) AS `re` ON `re`.`userId` = `users`.`id` 
LEFT JOIN (SELECT (CASE WHEN SUM(amount) IS NULL THEN 0 ELSE SUM(amount) END) AS sumDrAmount,`user_id` FROM `drawing` WHERE `status` = 2 GROUP BY `user_id`) AS `dr` ON `dr`.`user_id` = `users`.`id` 
