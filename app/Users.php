...
LEFT JOIN (SELECT (CASE WHEN SUM(amount) IS NULL THEN 0 ELSE SUM(amount) END) AS sumReAmount,`userId` FROM `recharges` WHERE `status` = 2 AND `payType` != 'adminAddMoney' GROUP BY `userId`) AS `re` ON `re`.`userId` = `users`.`id` 
